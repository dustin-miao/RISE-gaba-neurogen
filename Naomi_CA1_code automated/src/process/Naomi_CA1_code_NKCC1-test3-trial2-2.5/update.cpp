...
void calc_leak(Compartment * neuron, double time, double dt){  // this function calculates max na/katpase and leak currents for each ion
  ...
  if(time < dt) {
	...
	neuron->Icl_total = neuron->cl_kcc2 + 2.5*(neuron->cl_nkcc1);
...
void set_clleak(Compartment * neuron, double time, double h){
	if(time<h){
		neuron->Icl_total = 2.5*(neuron->cl_nkcc1) + neuron->cl_kcc2;
...
*/
  ...
  /* define total membrane current */
	...
	neuron->Icl_total = neuron->Iclleak + neuron->cl_kcc2 + neuron->Icl_gaba + neuron->Icl_gaba_syn + 2.5*(neuron->cl_nkcc1);
...
//  }
  ...
  //neuron->Icl_total = neuron->Iclleak + neuron->cl_kcc2 + neuron->cl_nkcc1; *2.5
...
//  }
  ...
  neuron->Icl_total = neuron->Iclleak + neuron->cl_kcc2 + 2.5*(neuron->cl_nkcc1);
